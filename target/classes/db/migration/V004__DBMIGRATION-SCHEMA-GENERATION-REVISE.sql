...
create table restaurante (
	id bigint not null auto_increment, 
    data_atualizacao datetime(2) not null, 
    data_cadastro datetime(2) not null, 
...
alter table produto add constraint FK_restaurante foreign key (restaurante_id) references restaurante (id);

alter table restaurante add constraint FK_cozinha foreign key (cozinha_id) references cozinha (id);

alter table restaurante add constraint FK_cidade foreign key (endereco_cidade_id) references cidade (id);
...
alter table usuario_grupo add constraint FK_grupo_ug foreign key (grupo_id) references grupo (id);

alter table usuario_grupo add constraint FK_usuario_ug foreign key (usuario_id) references usuario (id);
